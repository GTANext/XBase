...
bool IsSupportedModel(int model) {
    if (model < 90 || model > 150 || !CModelInfo::IsVehicleModel(model)) return false;
    switch (model) {
    case 124: case 125: case 126: case 140: case 141: case 147: case 150:
        return false;
    default:
        return true;
    }
...
bool GetLocked(void* vehicle) { return vehicle && AsVehicle(vehicle)->m_eDoorLock == CARLOCK_LOCKED_PLAYER_INSIDE; }
void SetLocked(void* vehicle, bool enable) {
    if (vehicle) AsVehicle(vehicle)->m_eDoorLock = enable ? CARLOCK_LOCKED_PLAYER_INSIDE : CARLOCK_UNLOCKED;
}
Types::ProofState GetProofState(void* vehicle) {
    ...
    state.nonPlayer = value->bOnlyDamagedByPlayer;
    return state;
}
void SetProofState(void* vehicle, const Types::ProofState& state) {
    ...
    value->bOnlyDamagedByPlayer = state.nonPlayer;
}
...
void WarpToSeat(void* vehicle, int seatIndex) {
    ...
    if (seatIndex <= 0) {
        ...
        player->WarpPedIntoCar(AsVehicle(vehicle));
    } else {
